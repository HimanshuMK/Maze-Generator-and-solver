...
bool SolveStack(int startX, int startY) // Change parameter names to avoid conflict
{
    stack<pair<Cell, pair<int, int>>> st;

    st.push(make_pair(maze[startY][startX], make_pair(startY, startX))); // Use startX and startY
    renderMaze();
    delay(100);
    ...
    while (!st.empty())
    {
        pair<Cell, pair<int, int>> u = st.top();
        st.pop();
        Cell ma = u.first;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                if (maze[newY][newX].type == End) // Check if the current cell is the end
                {
                    return true; // Path found
                }
                else
                {
                    st.push(make_pair(maze[newY][newX], make_pair(newY, newX)));
                }
            }
        }
    }
